        ...
        <>
            ...
            <div
                className={`${getInputHeight(
                    size
                )} flex gap-2 border border-slate-400 rounded p-1 overflow-auto w-full ${
                    label && 'mt-1'
                } ${disabled && 'opacity-60'} `}
            >
                {tags.map((tag, index) => (
                    <React.Fragment key={index}>
                        <>
                            {renderTag ? (
                                renderTag(tag, () => {
                                    !disabled && !readonly && removeTag(index)
                                })
                            ) : (
                                <div className="flex gap-3 items-center rounded-full bg-slate-300 p-1 px-2 text-sm h-[25px]">
                                    ...
                                    <div
                                        ...
                                        className={`${
                                            ...
                                            !readonly &&
                                            'cursor-pointer'
                                        } h-[20px] w-[20px] flex justify-center items-center rounded-full bg-slate-100`}
                                    >
                                        x
                                    </div>
                                </div>
